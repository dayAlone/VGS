	...
	<td>
		...
		foreach($arResult["arMap"] as $index => $arItem):?>
			...
			<?if (array_key_exists($index+1, $arResult["arMap"]) && $arItem["LEVEL"] < $arResult["arMap"][$index+1]["LEVEL"]):?>
				...
				<li><a href="<?=$arItem["FULL_PATH"]?>"><?=strip_tags(str_replace("<br/>", " ", html_entity_decode($arItem["NAME"])))?></a><?if ($arParams["SHOW_DESCRIPTION"] == "Y" && strlen($arItem["DESCRIPTION"]) > 0) {?><div><?=$arItem["DESCRIPTION"]?></div><?}?>
					<ul class="map-level-<?=$arItem["LEVEL"]+1?>">

			<?else:?>

					<li><a href="<?=$arItem["FULL_PATH"]?>"><?=strip_tags(str_replace("<br/>", " ", html_entity_decode($arItem["NAME"])))?></a><?if ($arParams["SHOW_DESCRIPTION"] == "Y" && strlen($arItem["DESCRIPTION"]) > 0) {?><div><?=$arItem["DESCRIPTION"]?></div><?}?></li>
